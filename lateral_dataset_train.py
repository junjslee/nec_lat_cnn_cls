...
class LateralDatasetTrain(Dataset):
    ...
    def process_row_crop(self, dcm_img):
        ...
        dcm_img_8u = cv2.normalize(dcm_img, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
...
#     def __init__(self, df, args, training=True):
#         """
#         For internal training/validation.
#         Expects df with columns: 'img_dcm' and 'binary_label' (0 or 1).
#         """
#         self.df = df.reset_index(drop=True) # self.df = df
#         self.training = training
...
#                         p=self.args.ela_percentage
#                     ),
#                     A.GaussNoise(var_limit=(self.args.gaussian_min, self.args.gaussian_max), p=self.args.gaussian_percentage),
#                 ], p=self.args.gaussian_percentage),
#                 A.OneOf([
#                     A.RandomGamma(gamma_limit=(self.args.gamma_min, self.args.gamma_max), p=self.args.gamma_percentage),
#                 ], p=0.5),
#                 A.CLAHE(clip_limit=self.args.clahe_cliplimit, p=0.5),
